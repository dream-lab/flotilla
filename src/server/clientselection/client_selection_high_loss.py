...
def client_selection(
    ...
    if round_no == 1:
        ...
        if num_clients == 0:
            print(
                "CLIENT_SELECTION.high_loss:: Number of clients to be selected from a tier came to be zero. Setting value to one."
            )
            num_clients = 1

        sorted_clients = sorted(latest_loss.items(), lambda x: x[1], reverse=True)
